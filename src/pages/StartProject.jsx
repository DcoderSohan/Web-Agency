            ...
            <form className="space-y-16" onSubmit={handleSubmit}>
              ...
              <section className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <label className="flex items-center gap-3 p-4 border-2 border-black cursor-pointer hover:bg-[#eeeeee] transition-colors has-[:checked]:bg-black has-[:checked]:text-white group relative">
                    <input className="absolute opacity-0" type="checkbox"/>
                    <span className="font-['JetBrains_Mono'] text-[12px] uppercase font-bold tracking-widest z-10 w-full text-center sm:text-left leading-relaxed text-black group-has-[:checked]:text-white">Technical Infrastructure</span>
                  </label>
                  <label className="flex items-center gap-3 p-4 border-2 border-black cursor-pointer hover:bg-[#eeeeee] transition-colors has-[:checked]:bg-black has-[:checked]:text-white group relative">
                    <input className="absolute opacity-0" type="checkbox"/>
                    <span className="font-['JetBrains_Mono'] text-[12px] uppercase font-bold tracking-widest z-10 w-full text-center sm:text-left leading-relaxed text-black group-has-[:checked]:text-white">System Architecture</span>
                  </label>
                  <label className="flex items-center gap-3 p-4 border-2 border-black cursor-pointer hover:bg-[#eeeeee] transition-colors has-[:checked]:bg-black has-[:checked]:text-white group relative">
                    <input className="absolute opacity-0" type="checkbox"/>
                    <span className="font-['JetBrains_Mono'] text-[12px] uppercase font-bold tracking-widest z-10 w-full text-center sm:text-left leading-relaxed text-black group-has-[:checked]:text-white">UI/UX Design</span>
                  </label>
                </div>
              </section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-4 border-2 border-black">
                      {['Q1', 'Q2', 'Q3', 'Q4'].map((q) => (
                        <label key={q} className={`border-r-2 border-black last:border-0 hover:bg-[#eeeeee] text-center py-4 cursor-pointer font-['JetBrains_Mono'] text-[14px] uppercase font-bold transition-all ${targetQuarter === q ? 'bg-black text-white' : 'text-black'}`}>
                          <input 
                            className="hidden" 
                            name="timeline" 
                            type="radio" 
                            value={q}
                            checked={targetQuarter === q}
                            onChange={() => setTargetQuarter(q)}
                          />
                          {q}
                        </label>
                      ))}
                  ...
                  <div className="space-y-4">
                    ...
                    <select className="w-full bg-transparent border-2 border-black text-black font-['JetBrains_Mono'] text-[14px] font-bold uppercase h-[60px] px-6 appearance-none focus:outline-none focus:border-b-4 focus:border-r-4 transition-all cursor-pointer">
              ...
              <div className="pt-8">
                <button 
                  className="w-full bg-black text-white font-['Syne'] text-[24px] md:text-[32px] font-bold py-6 px-6 uppercase border-2 border-black hover:bg-[#faf9f9] hover:text-black transition-all duration-300 group flex items-center justify-between" 
                  type="submit"
                >
